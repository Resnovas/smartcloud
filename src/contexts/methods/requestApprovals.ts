    ...
    for (let group in requestApprovals) {
        ...
        return groupConfig.condition.forEach(async (convention) => {
            ...
            if (await evaluator.call(this, convention, this.context.props)) {
                ...
                const body =
                    (groupConfig?.comment?.commentHeader == undefined ? "" : groupConfig?.comment?.commentHeader + "\n\n") +
                    ...
                    (groupConfig?.comment?.commentFooter == undefined ? "" : groupConfig?.comment?.commentFooter)
                await this.util.api.pullRequests.reviews.requestReviewers(this.context.props.ID, groupConfig!.reviewers).catch((error) => {
                    log(LoggingLevels.error, `Unable to automatically request approval`, error)
                })
                return await this.createComment.call(this, "Approvals", false, { event: "COMMENT", body }).catch((error) => {
                    log(LoggingLevels.error, `Unable to automatically request approval`, error)
                })
            } else {
